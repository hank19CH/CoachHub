...
onMounted(async () => {
  if (!isOwnProfile.value && route.params.username) {
    ...
    try {
      ...
      loadError.value = e?.message || 'Failed to load profile data'
    }
  }
})
...
<template>
  ...
  <div v-else-if="loadError" class="p-6 text-center">
    ...
    <p class="text-gray-900 font-medium mb-2">Couldn't load profile</p>
  ...
  <div v-else-if="displayProfile" class="pb-20">
    ...
    <div v-if="isOwnProfile" class="flex justify-end px-4 pt-3">
      <router-link
        to="/settings"
