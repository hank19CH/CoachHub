import{s as n}from"./index-B256TZ31.js";async function m(s,r,t){try{const{data:e,error:o}=await n.from("workout_completions").insert({assignment_id:s,athlete_id:r,completed_at:new Date().toISOString(),duration_minutes:t.durationMinutes,athlete_notes:t.athleteNotes,overall_rpe:t.overallRpe,has_pb:t.exerciseResults.some(d=>d.is_pb),caption:null,shared_exercise_ids:[],share_settings:{}}).select("id").single();if(o)throw o;if(t.exerciseResults.length>0){const d=t.exerciseResults.map(c=>({completion_id:e.id,exercise_id:c.exercise_id,sets_completed:c.sets_completed,reps_completed:c.reps_completed,weight_used_kg:c.weight_used_kg,duration_seconds:c.duration_seconds,distance_meters:c.distance_meters,rpe:c.rpe,is_pb:c.is_pb,notes:c.notes})),{error:u}=await n.from("exercise_results").insert(d);if(u)throw u}const{error:a}=await n.from("workout_assignments").update({status:"completed"}).eq("id",s);if(a)throw a;const i=t.exerciseResults.filter(d=>d.is_pb);return i.length>0&&await g(r,i,e.id),{success:!0,completionId:e.id}}catch(e){return console.error("Error completing workout:",e),{success:!1,error:e.message}}}async function g(s,r,t){try{for(const e of r){const{data:o}=await n.from("exercises").select("name").eq("id",e.exercise_id).single();if(!o)continue;let a=null,i=null;e.weight_used_kg&&e.weight_used_kg>0?(a="weight",i=e.weight_used_kg):e.reps_completed?(a="reps",i=parseInt(e.reps_completed)):e.duration_seconds&&e.duration_seconds>0?(a="time",i=e.duration_seconds):e.distance_meters&&e.distance_meters>0&&(a="distance",i=e.distance_meters),a&&i&&!isNaN(i)&&await n.from("personal_bests").upsert({athlete_id:s,exercise_name:o.name,pb_type:a,value:i,achieved_at:new Date().toISOString(),exercise_result_id:t},{onConflict:"athlete_id,exercise_name,pb_type"})}}catch(e){console.error("Error updating personal bests:",e)}}async function w(s,r,t,e){try{const{data:o}=await n.from("personal_bests").select("value").eq("athlete_id",s).eq("exercise_name",r).eq("pb_type",t).single();return o?t==="time"?e<o.value:e>o.value:!0}catch{return!0}}const h={async createAssignment(s){const{data:r,error:t}=await n.from("workout_assignments").insert({workout_id:s.workoutId,athlete_id:s.athleteId,coach_id:s.coachId,assigned_date:s.assignedDate,notes:s.notes||null,status:"pending"}).select().single();if(t)throw console.error("Error creating assignment:",t),new Error("Failed to create workout assignment");return r},async createBatchAssignments(s){const r=s.athleteIds.map(o=>({workout_id:s.workoutId,athlete_id:o,coach_id:s.coachId,assigned_date:s.assignedDate,notes:s.notes||null,status:"pending"})),{data:t,error:e}=await n.from("workout_assignments").insert(r).select();if(e)throw console.error("Error creating batch assignments:",e),new Error("Failed to create workout assignments");return t},async getCoachAssignments(s){const{data:r,error:t}=await n.from("workout_assignments").select(`
        *,
        workout:workouts (
          id,
          name,
      ...
      `).eq("athlete_id",s).eq("assigned_date",r).eq("status","pending").order("created_at",{ascending:!0});if(e)throw console.error("Error fetching today's assignments:",e),new Error("Failed to fetch today's assignments");return t||[]}};export{h as a,m as b,w as c};
